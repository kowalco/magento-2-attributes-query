...
class Text extends AbstractHelper
{
    ...
    public function Update($sku, $kod, $value, $store_id = 0)
    {
        ...
        return $this->tools->updateAtt($sku, $value, $kod, 'catalog_product_entity_text');
    }


    public function deleteValue($sku, $kod, $store_id = 0)
    {
        if (!$this->tools->checkIfSkuExists($sku)) return false;

        $this->tools->store_id = $store_id;
        return $this->tools->deleteAttrValue($sku,  $kod, 'catalog_product_entity_text');
    }
}
